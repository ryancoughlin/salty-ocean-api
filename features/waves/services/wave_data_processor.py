...
class WaveDataProcessor:
    ...
    def load_dataset(self, model_run: str, date: str) -> Optional[xr.Dataset]:
        ...
        # First check if we already have a valid dataset loaded
        ...
        try:
            ...
            if not forecast_files:
                # If we have no files for current run but have cached data, keep using it
                if self._cached_dataset is not None:
                    logger.warning("No new forecast files found, using existing cached data")
                    return self._cached_dataset
                logger.warning("No forecast files found and no cached data available")
                return None
    ...
    def close_dataset(self):
        """Close the cached dataset and clear cache."""
        if self._cached_dataset is not None:
            try:
