...
from core.model_time import get_latest_model_run
...
class WaveDataDownloader:
    def __init__(self, data_dir: str = settings.data_dir):
        ...
        self._download_state = {
            "last_attempt": None,
            "last_success": None,
            "retry_after": 3600,  # Default 1 hour retry
            "error_count": 0,
            "error_type": None
        }
            ...
            try:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        self._download_state["error_type"] = "forbidden"
                        self._download_state["retry_after"] = 3600  # Retry after 1 hour for 403
                        logger.error(f"Access forbidden (403) for {url} - NOAA API access denied")
                        return False
                    else:
    ...
    async def download_latest(self) -> bool:
        ...
        self._download_state["last_attempt"] = time.time()
        ...
        if not new_files:
            ...
            self._download_state["last_success"] = time.time()  # Consider this a success
            return True
        ...
        try:
            ...
            # Update download state based on results
            if success_count == len(new_files):
                logger.info(f"Successfully downloaded {success_count} forecast files")
                self._download_state["last_success"] = time.time()
                self._download_state["error_count"] = 0
                self._download_state["error_type"] = None
                return True
            else:
                # If we got any 403s, log it but don't treat as fatal if we have existing data
                if self._download_state["error_type"] == "forbidden":
                    logger.warning("NOAA API access denied (403) - will use existing data if available")
                    return False
                    
                logger.error(f"Only downloaded {success_count}/{len(new_files)} files successfully")
                self._download_state["error_count"] += 1
                return False
