...
const BEAUFORT_SCALE = [
  { speed: 1, description: "Calm", seaCondition: "Sea like a mirror" },
  {
    ...
    seaCondition: "Ripples without crests",
  },
  ...
  { speed: 63, description: "Storm", seaCondition: "Very high waves" },
  {
    ...
    seaCondition: "Exceptionally high waves",
  },
  {
    speed: 83,
    description: "Hurricane",
    seaCondition: "Air filled with foam and spray",
  },
];

// Get Beaufort scale description
const getBeaufortDescription = (windSpeed) => {
  const category = BEAUFORT_SCALE.find(
    (cat, index, arr) => windSpeed <= cat.speed || index === arr.length - 1
  );
  return category;
};
...
class NDBCService {
  ...
  parseValue(value, type) {
    ...
    switch (type) {
      case "WVHT":
        return formatWaveHeight(num);
      case "WSPD":
      case "GST":
        return formatWindSpeed(num);
      case "DPD":
      case "APD":
        return formatPeriod(num);
      case "MWD":
      case "WDIR":
        return formatDirection(num);
      case "WTMP":
      case "ATMP":
      case "DEWP":
        return formatTemperature(num);
      case "PRES":
        return formatPressure(num);
      default:
        return num;
    }
  }
  ...
  analyzeTrends(observations) {
    ...
    const firstValidWaveData = [...periods]
      .reverse()
      .find((p) => p.waves.height !== null);

    // Calculate changes only if we have valid data points
    const waveHeightChange =
      lastValidWaveData && firstValidWaveData
        ? lastValidWaveData.waves.height - firstValidWaveData.waves.height
        : null;
    const periodChange =
      lastValidWaveData && firstValidWaveData
        ? lastValidWaveData.waves.dominantPeriod -
          firstValidWaveData.waves.dominantPeriod
        : null;

    const windSpeedChange = last.wind.speed - first.wind.speed;
    const windDirChange =
      ((last.wind.direction - first.wind.direction + 540) % 360) - 180;
    ...
    if (lastValidWaveData) {
      const waveTrend = !waveHeightChange
        ? "steady"
        : Math.abs(waveHeightChange) < 0.5
        ? "steady"
        : waveHeightChange > 0
        ? "building"
        : "dropping";

      const periodTrend = !periodChange
        ? "steady"
      ...
      waveTrendDesc =
        `Waves ${waveTrend} at ${lastValidWaveData.waves.height}ft` +
        (waveHeightChange
          ? ` (${waveHeightChange > 0 ? "+" : ""}${waveHeightChange.toFixed(
              1
            )}ft)`
          : "") +
        (lastValidWaveData.waves.dominantPeriod
    ...
    const summary =
      `${waveTrendDesc}. ${beaufort.description} winds ${windTrend} at ${last.wind.speed}mph` +
    ...
    return {
      ...
      wind: {
        ...
        beaufort: beaufort,
        gustFactor: last.wind.gust
          ? (last.wind.gust - last.wind.speed).toFixed(1)
          : null,
      },
      timeSpan: {
        start: first.time,
  ...
  async fetchSpectralData(buoyId) {
    try {
      ...
      if (values.length < 15) {
        logger.warn(
          `Invalid spectral data format for buoy ${buoyId}, got ${values.length} columns, expected 15`
        );
        return null;
      }
      ...
      const spectralData = {
        ...
        waves: {
          ...
          swell: {
            ...
            direction:
              values[SPECTRAL_COLUMNS.SwD] === "MM"
                ? null
                : values[SPECTRAL_COLUMNS.SwD],
          },
          windWave: {
            height: this.parseValue(values[SPECTRAL_COLUMNS.WWH], "WVHT"),
            period: this.parseValue(values[SPECTRAL_COLUMNS.WWP], "WWP"),
            direction:
              values[SPECTRAL_COLUMNS.WWD] === "MM"
                ? null
                : values[SPECTRAL_COLUMNS.WWD],
          },
          steepness:
            values[SPECTRAL_COLUMNS.STEEPNESS] === "MM"
              ? null
              : values[SPECTRAL_COLUMNS.STEEPNESS],
        },
      };
      ...
      if (error.response?.status === 404) {
        ...
        logger.warn(
          `Error fetching spectral data for buoy ${buoyId}:`,
          error.message
        );
      }
      return null;
  ...
  createMarinerSummary(metData, spectralData) {
    ...
    if (spectralData?.waves) {
      ...
      if (hasSwell && hasWindWaves) {
        summary.push(
          `Mixed conditions with ${waves.windWave.height.toFixed(
            1
          )}ft wind waves ` +
            `(${waves.windWave.period}s) from the ${waves.windWave.direction} and ` +
            `${waves.swell.height.toFixed(1)}ft swell (${
              waves.swell.period
            }s) from the ${waves.swell.direction}`
        );
      } else if (hasSwell) {
        summary.push(
          `Clean ${waves.swell.height.toFixed(1)}ft swell from the ${
            waves.swell.direction
          } ` + `at ${waves.swell.period}s`
        );
      } else if (hasWindWaves) {
        summary.push(
          `${waves.steepness.toLowerCase()} ${waves.windWave.height.toFixed(
            1
          )}ft ` + `wind waves from the ${waves.windWave.direction}`
        );
      }
    } else if (metData.waves.height) {
      summary.push(
        `${metData.waves.height.toFixed(1)}ft waves at ${
          metData.waves.dominantPeriod
        }s`
      );
    }
