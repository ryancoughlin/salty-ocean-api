...
const { getOrSet, getCache } = require("../utils/cache");
...
async function getStationData(stationId) {
  try {
    // Get cache configs
    const buoyConfig = getCacheConfig("buoyData", stationId);

    // First try to get from cache only
    const cachedData = await getCache(buoyConfig.key);
    if (cachedData) {
      logger.info(`Serving cached data for station ${stationId}`);
      return cachedData;
    }
    ...
    if (stationInfo?.location?.coordinates) {
      try {
        ...
        // Try cache first
        forecast = await getCache(forecastConfig.key);
    ...
    logger.error(`Error fetching station data for ${stationId}:`, error);
    throw error;
  }
}
